...
def main():
    ...
    for point, runs in all_data.items():
        ...
        for run_index, run_data in enumerate(runs):
            ...
            interpolated_values = f_interp(average_data.index)

            # Calculate MAE
            mae = np.mean(np.abs(interpolated_values - average_data.values))
            mae_list.append(mae)

            datapoint_folder = os.path.join('linear', f'point_{point}')
            ...
            axs[0].plot(x_coords, y_coords, '-o', color='green')
            axs[0].plot(average_data.index, interpolated_values, '--', color='blue', label='Interpolated Line')
            ...
            # Full average plot with the copied subsampled line
            axs[1].scatter(average_data.index, average_data, color='red')
            axs[1].plot(average_data.index, interpolated_values, '--', color='blue', label='Interpolated Line')
            axs[1].set_title(f'Full Average vs. Interpolated, Run {run_index+1}, Datapoint index={point} - MAE: {mae:.4f}')
    ...
    mae_csv_path = os.path.join('linear', 'mae_summary.csv')
