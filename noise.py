...
def main():
    ...
    # Load the averages from compo_resA.csv
    ave_filepath = os.path.join(input_folder, 'compo_resA.csv')
    try:
        ave_df = pd.read_csv(ave_filepath)
    except Exception as e:
        print(f"Error reading {ave_filepath}: {e}")
        return

    for filename in os.listdir(input_folder):
        if filename.startswith('compo_res') and filename.endswith('.csv') and filename != 'compo_resA.csv':
            ...
            for point, group in grouped:
                ...
                datapoint_folder = os.path.join('noise', f'point_{point}')
    ...
    for point, data in all_data.items():
        ...
        mean_df = ave_df[ave_df['point'] == point]
        ...
        # Calculate and save the differences between each run and the average
        for i, run_df in enumerate(data):
            diff_df = run_df.copy()
            diff_df['Privacy cost difference'] = run_df['Privacy cost'] - mean_df.set_index('step').loc[run_df['step']]['Privacy cost'].values
            
            # Compute the sum of differences
            sum_diff = diff_df['Privacy cost difference'].sum()
            diff_df['Sum of differences'] = sum_diff
            
            diff_csv_filename = os.path.join('noise', f'point_{point}', f'diff_run_{i}_point_{point}.csv')
            diff_df.to_csv(diff_csv_filename, index=False)
            print(f'Difference CSV saved as {diff_csv_filename}')
        ...
        plot_filename = os.path.join('noise', f'point_{point}', 'average_plot.png')
        plt.savefig(plot_filename)
